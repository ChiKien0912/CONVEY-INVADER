...
def score_display():   
    if score==500 and star_game==False and game_active==False:
        score_state1 = game_font.render('WANNA TRY!',True,(240,154,196))
        score_state1_rect=score_state1.get_rect(center = (400,300.5))
        screen.blit(score_state1,score_state1_rect)
    else:
        score_surface = game_font.render(f'Score:{int(score)}',True,(240,154,196))
        score_rect=score_surface.get_rect(center = (400,155.5))
...
pygame.time.set_timer(make_obstacle,1500)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == make_obstacle and game_active:
            obstacle_rect_list=finish1.make_creep()
    ...
    obstacle_rect_list,result, attack = finish1.check_collision(obstacle_rect_list)
